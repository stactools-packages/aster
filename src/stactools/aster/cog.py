...
def _create_cog_for_sensor(
    ...
    bounds: List[float],
    crs: str,
    subdataset_info: List[Tuple[Any, int]],
) -> str:
    sensor_cog_href = os.path.join(output_dir, get_cog_filename(file_prefix, sensor))
...
def create_cogs(
    hdf_path: str, xml_metadata: XmlMetadata, output_path: str
) -> Dict[str, str]:
    ...
    for subdataset in subdatasets:
        ...
        if m is None:
            raise ValueError(
                f"Unexpected subdataset {subdataset} - is this a "
